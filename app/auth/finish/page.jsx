...
export default function FinishAuth() {
  ...
  useEffect(() => {
    ...
    if (mode === 'signup' || mode === 'recovery') {
      ...
      (async () => {
        ...
        const { data } = await supabase.from('users').select('onboarded').eq('id', user.id).single();
  ...
  return (
    <div className="min-h-screen grid place-items-center bg-gray-950 text-white p-6">
      <form onSubmit={handleSetPassword} className="w-full max-w-sm bg-gray-900 p-6 rounded-xl border border-gray-800 space-y-4">
