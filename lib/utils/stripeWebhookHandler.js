...
export async function handleStripeEvent(event) {
  ...
  try {
    if (event.type === "invoice.payment_succeeded") {
      ...
      // If the member was past_due and Stripe successfully collected payment,
      // restore them to active. Do not reactivate cancelled/suspended/terminated rows.
      try {
        const renewalSuccessUpdates = {
          renewal_pending: false,
          renewal_attempt_count: 0,
          last_renewal_attempt: payment_date,
        };
      
        if (membershipRow?.status === "past_due") {
          renewalSuccessUpdates.status = "active";
        }
    ...
    if (event.type === "invoice.payment_failed") {
      ...
      const currentStatus = String(membershipRow?.status || "").toLowerCase();
      ...
      if (hasMembership) {
        if (currentStatus === "scheduled") {
          try {
            ...
            console.error(
              "❌ Failed to update scheduled membership after failed activation charge:",
              schedErr.message
            );
          }
        } else if (["active", "past_due"].includes(currentStatus)) {
          try {
            await updateMembershipByUserAndSubscription(
              supabase,
              userId,
          ...
          console.log(
            `🟡 Skipping past_due update for membership ${membershipId}; current status is ${
    ...
    if (event.type === "customer.subscription.deleted") {
      ...
      try {
        await updateMembershipByUserAndSubscription(
          ...
          {
            status: "cancelled",
            expired_on: membershipRow?.expired_on || null,
            expires_at: membershipRow?.expires_at || nowLocal,
            grace_ends_at: membershipRow?.grace_ends_at || nowLocal,
            next_payment_date: null,
            auto_renewal_enabled: false,
            renew_at_discounted_rate: false,
            renewal_pending: false,
            last_renewal_attempt: nowLocal,
            cancelled_on: membershipRow?.cancelled_on || nowLocal,
            cancelled_by_role: membershipRow?.cancelled_by_role || "stripe",
            cancel_reason:
              membershipRow?.cancel_reason || `Stripe subscription deleted: ${reason}`,
          }
        );
      ...
      await logMembershipEvent({
        ...
        plan: null,
        durationLabel: null,
        contractEndDate: membershipRow?.contract_end_date || null,
        nextPaymentDate: null,
        expiresAt: membershipRow?.expires_at || nowLocal,
        graceEndsAt: membershipRow?.grace_ends_at || nowLocal,
        expiredOn: membershipRow?.expired_on || null,
