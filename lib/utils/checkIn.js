  ...
  try {
    ...
    const watchId = navigator.geolocation.watchPosition(
      (position) => {
        ...
        if (accuracy <= 50) {
          finishCheckIn(bestPosition, sessionData.session.access_token, router, toastId, watchId);
        }
      },
      (error) => {
    ...
    setTimeout(() => {
      navigator.geolocation.clearWatch(watchId);
      if (bestPosition && bestAccuracy <= 75) {  // fallback threshold
        finishCheckIn(bestPosition, sessionData.session.access_token, router, toastId, null);
      } else {
        dismissToast(toastId);
...
async function finishCheckIn(position, accessToken, router, toastId, watchId) {
  ...
  const { latitude, longitude, accuracy } = position.coords;
  console.log(`✅ Using best GPS fix: ${latitude} ${longitude} Accuracy: ${accuracy} meters`);
  ...
  const response = await fetch("/api/check-in", {
    ...
    body: JSON.stringify({ latitude, longitude, accuracy, method: "geolocation" }),  // ✅ include accuracy
  });
