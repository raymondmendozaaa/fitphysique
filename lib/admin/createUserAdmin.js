...
import { fetchUserByEmail, insertUser } from "@/lib/db/users";
...
export async function createUserAdmin({
  ...
  try {
    existingProfile = await fetchUserByEmail(supabaseAdmin, email, "id");
  } catch (existingErr) {
    throw new Error(`users lookup failed: ${existingErr.message}`);
  }
  ...
  try {
    await insertUser(supabaseAdmin, {
    ...
    throw new Error(`insert into users failed: ${insertErr.message}`);
  }
