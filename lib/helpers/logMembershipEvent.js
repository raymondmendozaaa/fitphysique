...
export async function logMembershipEvent({
  ...
  const needPlanMeta = ["created", "renewed", "activated"].includes(eventType);
  ...
  const { error } = await supabase.from("memberships_logs").insert({
    ...
    expired_on: expiredOn ? toLocalISOString(new Date(expiredOn)) : null,
    cancelled_on: cancelledOn ? toLocalISOString(new Date(cancelledOn)) : null,
